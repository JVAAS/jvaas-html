...
class HtmlDslGenerator {
	...
	init {
		...
		address.children =
			flowContent.minus(address).minus(*headingContent).minus(*sectioningContent).minus(header).minus(footer)
				.toMutableList()
		...
		dt.children =
			flowContent.minus(header).minus(footer).minus(*sectioningContent).minus(*headingContent).toMutableList()
	...
	companion object {
		...
		private fun Attribute.generateClassName(): String {
			return "Attr" + (this.generates ?: this.tag)
				.replace("-", " ")
				.replace("`", " ")
				.replaceFirst("on", "on ")
				.split(" ")
		...
		private fun Element.generateClassName(): String {
			return "Funt" + (this.generates ?: this.tag)
				.replace("-", " ")
				.replace("`", " ")
				.replaceFirst("on", "on ")
				.split(" ")
		...
		fun generateFunterfaces() {
			HtmlDslGenerator().let {
				...
				elements.forEach { el ->
					...
					val tag = el.generateClassName()

					File(
						"$path/src/main/kotlin/io/jvaas/dsl/html/funtribute/$tag.kt"
					).printWriter().use { out ->
						out.println("""package io.jvaas.dsl.html.funtribute""")
						out.println("")
						out.println("import io.jvaas.dsl.html.element.${el.tag.toUpperCase()}")
						out.println("")
						out.println("// generated by HtmlDslGenerator.kt")
						out.println("")
						out.println("interface $tag : Funt {")
						out.println("")
						out.println(
							"""
							\tfun ${el.generates ?: el.tag}(
							""".trimIndent().replace("""\t""", "\t")
						)
						val childParams = mutableListOf<String>()
						val childAttributes =
							el.attributes.distinctBy { it.generates ?: it.tag }.filter { !it.deprecated }
								.filter { it.standardized }.filter { !it.experimental }
								.filter { it.implemented }.sortedBy { it.tag }
						...
						if (!el.selfClosing) {
							childParams.add(
								"""
								 \t\tinit: (${el.tag.toUpperCase()}.() -> Unit)? = null
								""".trimIndent()
							)
						}
						...
						out.println("""\t) = ${el.tag.toUpperCase()}().let {""".makeTabs())
		...
		fun generateAttrInterfaces() {
			HtmlDslGenerator().let {
				...
				elements.forEach { el ->
					...
					el.attributes.distinctBy { it.generates ?: it.tag }.filter { !it.deprecated }
						.filter { it.standardized }.filter { !it.experimental }.filter { it.implemented }
						.sortedBy { it.tag }.toList().forEach { attr ->
							attributeMap.putIfAbsent(attr.tag, attr)
						}
				}
				...
				attributeMap.values.forEach { attr ->
					...
					val tag = attr.generateClassName()

					File(
						"$path/src/main/kotlin/io/jvaas/dsl/html/attribute/$tag.kt"
					).printWriter().use { out ->
						out.println("""package io.jvaas.dsl.html.attribute""")
						out.println("")
						out.println("// generated by HtmlDslGenerator.kt")
						out.println("")
						out.println(
							"""
							interface $tag : Attr {
								var ${attr.generates ?: attr.tag}: String?
									get() = attributes["${attr.tag}"]
									set(value) {
										value?.let {
											attributes["${attr.tag}"] = it
										}
									}
							}	
						""".trimIndent()
			...
			HtmlDslGenerator().let {
				...
				elements.forEach { el ->
					...
					File(
						...
						out.println("import io.jvaas.dsl.html.attribute.*")
						out.println("import io.jvaas.dsl.html.funtribute.*")
						out.println("")

						// extract attributes and children
						val attributes = el.attributes.distinctBy { it.generates ?: it.tag }.filter { !it.deprecated }
							.filter { it.standardized }.filter { !it.experimental }.filter { it.implemented }
							.sortedBy { it.tag }.toMutableList()
						...
						val ei = mutableListOf<String>()
						attributes.map { a -> a.generateClassName() }.forEach { a ->
							ei.add(a)
						}
						children.map { c -> c.generateClassName() }.forEach { c ->
							ei.add(c)
						}
						ei.add("Tag(")

						// add interfaces and classes
						ei.forEachIndexed { index, e ->

							val trailingComma = if (index == ei.lastIndex) {
								""
							} else {
								", "
							}
							...
							if (index == 0) {
								out.println(""") : $e$trailingComma """.trimMargin())
							} else {
								out.println("""\t$e$trailingComma """.trimMargin().makeTabs())
							}
						}
